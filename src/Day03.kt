fun main() {
    fun part1(input: List<String>): Int {
        fun hasNeighbouringSymbol(str: String, range: IntRange): Boolean {
            ...
            val safeRange = (range.first - 1).coerceIn(str.indices)..(range.last + 1).coerceIn(str.indices)
            return str.substring(safeRange).contains("[^0-9.]".toRegex())
        }
        
        val regex = "\\d+".toRegex()
        fun findNumbers(str: String): List<Pair<String, IntRange>> = regex.findAll(str)
            .map { match -> match.value to match.range }
            .toList()
        
        val partNumbers = input.mapIndexed { i, str ->
            val above = input.getOrNull(i-1) ?: ""
            val below = input.getOrNull(i+1) ?: ""
            findNumbers(str).filter { (_, range) ->
    ...
    fun part2(input: List<String>): Int {
        ...
        fun findGears(str: String): List<Int> = gearRegex.findAll(str)
        ...
        val gears = input.flatMapIndexed { i, str ->
            findGears(str).map {
                findNeighbours(
                    i = it,
                    input = str,
